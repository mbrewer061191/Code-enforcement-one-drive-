...
const CERT_MAIL_QUEUE_KEY = 'cert-mail-queue-v1';
...
export const getCertMailQueue = (): string[] => {
    try {
...
const saveCertMailQueue = (queue: string[]) => {
    localStorage.setItem(CERT_MAIL_QUEUE_KEY, JSON.stringify(queue));
};

export const addToCertMailQueue = (caseId: string) => {
...
export const removeFromCertMailQueue = (caseId: string) => {
    let queue = getCertMailQueue();
    queue = queue.filter(id => id !== caseId);
    saveCertMailQueue(queue);
};
